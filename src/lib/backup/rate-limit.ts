...
export async function checkBackupRateLimit(userId: string): Promise<{ allowed: boolean; retryAfterSeconds?: number }> {
  const db = getAdminDb();
  ...
  const lastMs = lastBackup.toDate?.()?.getTime() ?? new Date(lastBackup).getTime();
