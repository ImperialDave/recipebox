...
export default function JoinPage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setJoined(true);
      toast.success(result.alreadyMember ? "You're already a member!" : "Welcome to the family!");
    ...
    <div className="min-h-screen flex items-center justify-center bg-cream-100 p-4">
      <Card className="w-full max-w-md text-center">
        ...
        <CardContent className="space-y-4">
          <code className="block px-4 py-3 bg-cream-200 rounded-xl font-mono text-2xl tracking-widest">
            {code}
          </code>
          {joined ? (
            <p className="text-sage-600 font-medium">Redirecting to your group...</p>
          ) : (
            <>
